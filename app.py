...
Streamlit Application - Direct Model Loading (No Databricks Required)
...
def predict(text):
    ...
    Returns: (condition, confidence_tier, top3_predictions)
    """
    if not MODEL_LOADED:
    ...
    top3 = [(LABEL_ENCODER.classes_[idx], probs[idx]) for idx in top3_indices]
    
    # Primary prediction
    pred_idx = top3_indices[0]
    condition = LABEL_ENCODER.classes_[pred_idx]
    top1_prob = probs[pred_idx]
    top2_prob = probs[top3_indices[1]] if len(top3_indices) > 1 else 0
    ...
    return condition, confidence_tier, top3
...
def main():
    ...
    if not MODEL_LOADED:
        st.error("⚠️ Model files not found. Please ensure these files are in the app directory:")
    ...
    with col1:
        if st.button("🦴 Fracture"):
            st.session_state.example = "Patient fell from height, severe pain in arm, visible swelling, cannot move the limb, possible broken bone"
    with col2:
        if st.button("🔥 Burn"):
            st.session_state.example = "Patient has burn injury with blistering, severe pain, red and moist skin on hand from hot water"
    with col3:
        if st.button("🐕 Animal Bite"):
            st.session_state.example = "Patient presents with bite wound, puncture marks, bleeding, swelling from dog attack on leg"
    with col4:
        if st.button("🤒 Malaria"):
            st.session_state.example = "Patient has high fever, chills, sweating, headache, body aches, nausea, fatigue"
    
    # Check for example in session state
    if 'example' in st.session_state and st.session_state.example:
        symptoms_input = st.session_state.example
        st.session_state.example = None  # Clear after use
    ...
    if st.button("🔍 **Analyze Symptoms**", type="primary", use_container_width=True):
        if symptoms_input:
            with st.spinner("🔄 Analyzing symptoms..."):
                condition, confidence_tier, top3 = predict(symptoms_input)
            ...
            if condition:
                ...
                cols = st.columns(3)
                for i, (cond, prob) in enumerate(top3):
                    with cols[i]:
                        st.metric(
                            label=f"#{i+1}",
                            value=cond,
